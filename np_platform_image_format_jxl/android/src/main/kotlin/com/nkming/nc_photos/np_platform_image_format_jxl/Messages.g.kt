...
interface MyHostApi {
  fun load(filepath: String, w: Long?, h: Long?, callback: (Result<Image>) -> Unit)
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            api.load(filepathArg, wArg, hArg) { result: Result<Image> ->
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            val wArg = args[1] as Long?
            val hArg = args[2] as Long?
            api.loadBytes(bytesArg, wArg, hArg) { result: Result<Image> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(wrapError(error))
              } else {
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val filepathArg = args[0] as String
            api.loadMetadata(filepathArg) { result: Result<Metadata?> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(wrapError(error))
              } else {
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            api.save(imgArg, filepathArg) { result: Result<Boolean> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(wrapError(error))
              } else {
