...
class FormatConverter(
     ...
     */
    fun convert(
        uri: Uri, exportDir: File, format: Format, quality: Int, downsizeMp: Double?
    ): File? {
        ...
        try {
            contentResolver.openFileDescriptor(uri, "r").use {
                ...
                val result = measureTime(TAG, "[convert] Elapsed") {
                    convertNative(
                        it.fd, outFile.absolutePath, format.value, quality, downsizeMp ?: -1.0
                    )
                }
                if (result != 0) {
                    logE(
                        TAG, "Failed to convert file uri, result: $result, uri: $uri"
                    )
                    outFile.delete()
                    return null
                }
