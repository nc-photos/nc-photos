...
class _Bloc extends Bloc<_Event, _State>
  ...
  Future<void> _onInit(_Init ev, _Emitter emit) async {
    ...
    try {
      unawaited(_importPotentialSharedAlbum().then((value) {
        if (value.isNotEmpty) {
      ...
      unawaited(accountController.serverController.status.first.then((value) {
  ...
  Future<List<Album>> _importPotentialSharedAlbum() async {
    ...
    try {
      return await ImportPotentialSharedAlbum(c)(
          account, accountController.accountPrefController.shareFolderValue);
    } catch (e, stacktrace) {
      _log.shout(
