...
class _Bloc extends Bloc<_Event, _State> {
  ...
  Future<void> _initMetadata(_Emitter emit) async {
    final metadataGetter =
        AnyFileContentGetterFactory.metadata(file, c: c, account: account);
    SizeInt? size;
    int? byteSize;
    String? model;
    double? fNumber;
  ...
  Future<void> _initTag(_Emitter emit) async {
    final getter =
        AnyFileContentGetterFactory.tag(file, c: c, account: account);
  ...
  Future<void> _initCapability(_Emitter emit) async {
    ...
    final collectionAdapter = fromCollection
        ?.let((e) => CollectionAdapter.of(c, account, e.collection));
    ...
    emit(state.copyWith(
      ...
      canSetAs: getRawPlatform() == NpPlatform.android &&
  ...
  Future<void> _onSetAlbumCover(_SetAlbumCover ev, _Emitter emit) async {
    ...
    try {
      ...
      _log.shout(
          "[_onSetAlbumCover] Failed while updating album", e, stackTrace);
      emit(state.copyWith(
