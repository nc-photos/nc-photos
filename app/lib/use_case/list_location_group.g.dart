// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'list_location_group.dart';

// **************************************************************************
// NpLogGenerator
// **************************************************************************

extension _$ListLocationGroupNpLog on ListLocationGroup {
  // ignore: unused_element
  Logger get _log => log;

  static final log = Logger("use_case.list_location_group.ListLocationGroup");
}

// **************************************************************************
// ToStringGenerator
// **************************************************************************

extension _$LocationGroupToString on LocationGroup {
  String _$toString() {
    // ignore: unnecessary_string_interpolations
    return "LocationGroup {name: $name, countryCode: $countryCode, count: $count, latestFileId: $latestFileId, latestDateTime: $latestDateTime, latestFileMime: $latestFileMime, latestFileRelativePath: $latestFileRelativePath}";
  }
}

extension _$LocationGroupResultToString on LocationGroupResult {
  String _$toString() {
    // ignore: unnecessary_string_interpolations
    return "LocationGroupResult {name: [length: ${name.length}], admin1: [length: ${admin1.length}], admin2: [length: ${admin2.length}], countryCode: [length: ${countryCode.length}]}";
  }
}
