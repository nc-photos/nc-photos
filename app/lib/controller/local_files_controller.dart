...
class LocalTimelineStreamEvent {
  const LocalTimelineStreamEvent({
    required this.data,
  });
...
class LocalFilesSummaryStreamEvent {
  const LocalFilesSummaryStreamEvent({
...
class LocalFilesControllerImpl implements LocalFilesController {
  ...
  Future<void> queryTimelineByDateRange(DateRange dateRange) async {
    try {
      ...
      _log.severe(
          "[queryTimelineByDateRange] Uncaught exception", e, stackTrace);
  ...
  Future<void> _initSummary() async {
    ...
    _summaryStreamController.add(LocalFilesSummaryStreamEvent(
      summary: results,
    ));
  }
  ...
  final _dataStreamController =
      BehaviorSubject.seeded(_LocalFilesStreamEvent(files: {}));
  final _dataErrorStreamController =
      StreamController<ExceptionEvent>.broadcast();

  final _timelineStreamController =
      BehaviorSubject.seeded(const LocalTimelineStreamEvent(data: {}));
...
class _LocalFilesStreamEvent implements LocalFilesStreamEvent {
  _LocalFilesStreamEvent({
    required this.files,
    Lazy<List<LocalFile>>? dataLazy,
  }) : dataLazy = dataLazy ?? (Lazy(() => files.values.toList()));

  _LocalFilesStreamEvent copyWith({
    Map<String, LocalFile>? files,
  }) {
