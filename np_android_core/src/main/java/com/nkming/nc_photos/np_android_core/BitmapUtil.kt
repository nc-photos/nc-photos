    ...
    companion object {
        fun loadImageFixed(
            ...
            val subsample = calcBitmapSubsample(
                opt.outWidth, opt.outHeight, targetW, targetH, BitmapResizeMethod.FILL
            )
            if (subsample > 1) {
                logD(
                    TAG,
            ...
            val bitmap = decodeImage(context, uri, outOpt)
            if (subsample > 1) {
        ...
        fun loadImage(
            ...
            return if (isJxl) {
                loadJxlImage(
                    context, uri, targetW, targetH, resizeMethod, isAllowSwapSide, shouldUpscale,
        ...
        private fun loadJxlImage(
            ...
            val bytes = openUriInputStream(context, uri)!!.use { it.readBytes() }
            val imgSize = JxlCoder.getSize(bytes)!!
            val shouldSwapSide =
                isAllowSwapSide && imgSize.width != imgSize.height && (imgSize.width >= imgSize.height) != (targetW >= targetH)
            ...
            val result = JxlCoder.decodeSampled(
                bytes, dstW, dstH, PreferredColorConfig.RGBA_8888, scaleMode
            )
        ...
        private fun fixOrientation(
            ...
            return try {
                openUriInputStream(context, uri)!!.use {
                    ...
                    val orientation = iExif.getAttributeInt(ExifInterface.TAG_ORIENTATION, 1)
                    logI(
                        TAG, "[fixOrientation] Input file orientation: $orientation"
                    )
                ...
                logE(
                    TAG, "[fixOrientation] Failed fixing, assume normal orientation", e
                )
                bitmap
            }
        }
